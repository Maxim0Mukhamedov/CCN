...
int main(int argc, char **argv ) {
    ...
    double conValue = 0.1;
    std::string saveDirectory = "./test_output";
    std::string inputDirectory = "./test_input/test_image.jpg";
    for (int i = 1; i < argc; i ++) {
        if (static_cast<std::string>(argv[i]) == "-S") {
            ...
            std::cout << "next param to use this way: -[param]=[option]" << '\n';
            std::cout << "-v : change the converges value" << '\n';
            std::cout << "-i : change input directory" << '\n';
            std::cout << "-s : change output directory" << '\n';
            std::cout << "-h : show params meaning" << std::endl;
            return 0;
